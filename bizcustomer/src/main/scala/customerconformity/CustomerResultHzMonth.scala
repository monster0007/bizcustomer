...
//    val timeList=List("2016-10","2016-11","2016-12","2017-01","2017-02","2017-03","2017-04","2017-05","2017-06","2017-07","2017-08",
...
//    )
    val timeList=List("2019-05")
    timeList.foreach(dat=>{
      val num=dat
      val date=num.replace("-","")
      val customreDf=getResult(spark,num)
      customreDf.show()
      val path="hdfs://nameservice1/user/hive/warehouse/sys_customer_conformity.db/sys_cus_hz_"+date
  ...
  def getResult(spark: SparkSession,num:String): DataFrame = {
    ...
    spark.sql(
      """
        |create table if not exists sys_customer_conformity.sys_cus_hz_"""
        .stripMargin+date+"""
        |(customerid bigint,
        |createtime date,
        |leasttime date,
    ...
    val data=spark.sql(
      """
        ...
        |from sys_customer_conformity.dm_custom_month m
        |join sys_customer_conformity.cus_consume c on m.customerid=c.customerid and m.selfflag=c.is_own
        |left join sys_customer_conformity.customer_info i on m.customerid=i.id

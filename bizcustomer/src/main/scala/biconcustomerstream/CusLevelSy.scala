...
object CusLevelSy {
  def main(args: Array[String]): Unit = {
    ...
    val mon = getAimMonth(0)//今天的月份
    val res = getLevel(spark, month,nityDay,day)
    val path2 = "hdfs://nameservice1/user/hive/warehouse/sys_customer.db/cus_consume_sy/month=" + mon
  ...
  def getLevel(spark: SparkSession, month: String,nityday:String,day: String): DataFrame = {
    ...
    val nordata = spark.sql(
      """
        |select f.customerid,sum(amount)/3 consumemoney,g.is_own,f.accountid from sys_customer.sys_customer_salefact f
      ...
      """.stripMargin+"\""+month+"\""+" and sdate between "+"\""+nityday+"\""+" and "+"\""+day+"\""+
        " group by f.customerid,g.is_own,f.accountid"
    )
    ...
    val adddata = spark.sql(
      """
        |select f.customerid,sum(amount) consumemoney,is_own,f.accountid from sys_customer.sys_customer_salefact f
        |join sys_customer.dm_custom_month_sy m on f.customerid=m.customerid and f.accountid=m.accountid
        |join sys_customer.pcd_goods g on f.goodsid=g.goodsid

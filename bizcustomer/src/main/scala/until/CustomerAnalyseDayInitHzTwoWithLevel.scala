...
/**
  * 这个不用
  */
object CustomerAnalyseDayInitHzTwoWithLevel {
  ...
  def updateCusromerWithTime(spark:SparkSession,baseData:DataFrame,todayData:DataFrame):DataFrame={
    //计算当天购买了的客户,更新最近时间
   ...
   val cusWithTime=crossCus.union(ownCus).union(comCus)
    ...
    val crossCusFlag=spark.sql(
      """
        ...
        |end as addflag,
        |case
        |when datediff(currenttime,o_leasttime)<=90 and datediff(o_leasttime,o_lasttime)>0 and datediff(o_leasttime,o_lasttime)<=90 then 1
        |else 0
        |end as norflag,
        |case
    ...
    val resTmp=crossCusFlag.union(ownCusFlag).union(comCusFlag1).union(comCusFlag2).union(overNity1).union(overNity2).union(overNityCom)
    resTmp.createOrReplaceTempView("cusFlag")

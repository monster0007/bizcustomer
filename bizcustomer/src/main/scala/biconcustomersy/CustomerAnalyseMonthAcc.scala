...
/**
  ...
  * 月的客户自有标志取最后一天
  */
object CustomerAnalyseMonthAcc {
...
//    val month=getMonth(args(1).toInt)
  ...
  def getdata(spark:SparkSession,month:String,day:String): DataFrame ={
    ...
    val delData=spark.sql(
      """
        |select customerid,createtime,lasttime,leasttime,0 addflag,0 norflag,1 delflag,accountid from sys_customer.dm_custom_sy where delflag=1 and currenttime="2019-07-31"
      """.stripMargin)
//    val delData=spark.sql(
//      """

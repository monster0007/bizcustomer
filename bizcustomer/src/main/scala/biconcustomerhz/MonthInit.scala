...
object MonthInit {
  def main(args: Array[String]): Unit = {
    ...
    //val path="hdfs://nameservice1/user/hive/warehouse/sys_customer.db/dm_custom_month/month="+month
    val path="hdfs://nameservice1/user/hive/warehouse/sys_customer.db/dm_custom_month/month=2008-08"
  ...
  def getPreMon(spark:SparkSession,predate:String,firstDate:String,lastDate:String):DataFrame={
    ...
    val preData = spark.sql(
      """
        |select customerid,min(sdate) premintime,max(sdate) premaxtime from sys_customer.sys_customer_salefact f
        |join sys_customer.pcd_goods g on f.goodsid=g.goodsid
        |where g.is_own=1
        |and sdate between "2016-10-01" and """
        .stripMargin + "\"" + predate + "\"" + " group by customerid")

    //当前购买的商品信息
    val curData=spark.sql(
      """
        |select customerid,min(sdate) curmintime,max(sdate) curmaxtime from sys_customer.sys_customer_salefact f
        |join sys_customer.pcd_goods g on f.goodsid=g.goodsid
        |where g.is_own=1
        |and sdate between
      """.stripMargin+"\""+firstDate+"\""+"and"+"\""+lastDate+"\"" + " group by customerid"
    )

...
object MonthLevelInit {
  def main(args: Array[String]): Unit = {
    val spark=SparkSession.builder().appName("month_level_init").master("yarn").enableHiveSupport().config("file.encoding", "UTF-8").getOrCreate()
    ...
    //val path2 = "hdfs://nameservice1/user/hive/warehouse/sys_customer.db/cus_consume/month="+curmonth  //这个是正常使用的
    val path2 = "hdfs://nameservice1/user/hive/warehouse/sys_customer.db/cus_consume/month=2019-05"//这个是为了单独生成消费层次的，特殊
  ...
  */
  def monthLevel(spark:SparkSession,nityday:String,lastday:String,month:String):DataFrame={
    ...
    val nordata = spark.sql(
      """
        |select f.customerid,sum(amount)/3 consumemoney,1 is_own from sys_customer.sys_customer_salefact f
...
println(nordata.count+"     11111111111111111111111111111111")
    //新增客户的计算
    val adddata = spark.sql(
      """
        |select f.customerid,sum(amount) consumemoney,1 is_own from sys_customer.sys_customer_salefact f
        |join sys_customer.dm_custom_month m on f.customerid=m.customerid
        |join sys_customer.pcd_goods g on f.goodsid=g.goodsid
        |where m.addflag=1 and m.month=""".stripMargin +"\""+month+"\""+" and substr(sdate,1,7)="+"\""+month+"\""+"group by f.customerid"
    )
    println(adddata.count()+"            2222222222222222222222222")
    ...
    val norDel = spark.sql(
      """
        ...
        |from consume_level
      """.stripMargin)
println(nordata.count()+"   333333333333333333333333333333333333")
    /*
   流失客户的计算，直接给个消费层次0
    */

...
object CustomerAnalyseConLevel {
  def main(args: Array[String]): Unit = {
    ...
    val path2 = "hdfs://nameservice1/user/hive/warehouse/sys_customer.db/cus_consume/month=" + "2019-04"
  ...
  def getLevel(spark: SparkSession, month: String, day: String): DataFrame = {
    ...
    val nordata = spark.sql(
      """
        ...
        |where dcm.month="2019-03" and dcm.norflag=1 and dcm.addflag=0
        |and sdate between "2018-12-31" and "2019-03-31"
...
//    )
    ...
    val adddata = spark.sql(
      """
        |select scs.customerid,sum(amount) consumemoney from sys_customer.sys_customer_salefact scs
        |join sys_customer.dm_custom_month dcr on scs.customerid=dcr.customerid
        |where dcr.month="2019-03"  and dcr.addflag=1
        |and substr(sdate,1,7)="2019-03"
      """.stripMargin+
        "group by scs.customerid"
    )
    ...
    val norDel = spark.sql(
      ...
      """.stripMargin)
    /*
    流失客户的计算，计算方式为拿上个月的消费层次
     */
    val deldata = spark.sql(
      """
        |select c.customerid,c.consumelevel from sys_customer.cus_consume c join  sys_customer.dm_custom_month d
        |on c.customerid=d.customerid
        |where c.month="2019-03"
        |and d.month="2019-03" and d.delflag=1
      """.stripMargin)

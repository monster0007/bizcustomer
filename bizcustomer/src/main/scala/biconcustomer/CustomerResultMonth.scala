...
object CustomerResultMonth {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("writToCusDemssionMonth").master("yarn").enableHiveSupport().config("file.encoding", "UTF-8").getOrCreate()
    val sc = spark.sparkContext
    val num=args(0).toString
    val date=num.replace("-","")
    val customreDf=getResult(spark,date)
  ...
  def getResult(spark: SparkSession,date:String): DataFrame = {
    spark.sql(
      """
